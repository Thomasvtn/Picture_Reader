...
void calculweigth(layer *self,double *input)
{
	...
	for(int i=0; i<ligne;i++)
	{
		for(int j=0; j<colone;j++)
		{
			...
			self->weight[i*colone+j] += input[j]*delta*1;//*0.3;
			self->bias[i] += delta;
		}
	}
}
...
void forward(layer *self,double *input,double *output)
{
	...
	for(int copiewi = 0; copiewi < self -> size; copiewi++)
	{
		self -> weightsum[copiewi] = output[copiewi];
	}
...
void init(layer *self,int newsize,int newinput_size)
{
	...
	for(int i=0; i < newinput_size;i++)
	{
		...
		{
		self -> weight[newsize*i+j] =(rand()/(double)RAND_MAX)*(-1)+0.5;
		}
	}
	self -> size = newsize;
	...
	self -> weightsum = calloc(newsize,sizeof(double));
	self -> out = calloc(newsize,sizeof(double));
	for(int i=0; i< newsize;i++)
		self -> bias[i]=(rand()/(double)RAND_MAX)*(-1)+0.1;
}
